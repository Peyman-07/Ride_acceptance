...
def generic_generator(generator, n):
    # to create multiple s/vehicles/etc
...
def networkstats(inData):
    ...
    ret = DotMap({'center': nearest, 'radius': inData.skim[nearest].quantile(0.75),
                  'central_radius':central_radius,'central_nodes': central_nodes})
    return ret
...
def generate_vehicles(_inData, nV):
    ...
    vehs['part_time'] = vehs.apply(lambda x: 1 if random.random() > 0.5 else 0, axis=1) #f#
    vehs['beginner'] = vehs.apply(lambda x: 1 if random.random() > 0.5 else 0, axis=1) #f#
    vehs['gender'] = vehs.apply(lambda x: 1 if random.random() > 0.5 else 0, axis=1) #f#
    vehs['satisfied'] = vehs.apply(lambda x: 1 if random.random() > 0.5 else 0, axis=1) #f#
    vehs['educated'] = vehs.apply(lambda x: 1 if random.random() > 0.5 else 0, axis=1) #f#
...
def generate_demand(_inData, _params=None, avg_speed=False):
    ...
    df['rate'] = df.apply(lambda x: random.uniform(3.5, 5), axis=1)  #f#
    ...
    while len(requests[requests.dist <= 1000]) > 0: #f#
        requests.origin = requests.apply(lambda request: (distances.sample(1, weights='p_origin').index[0]
                                                          if request.dist <= 1000 else
                                                          request.origin),
                                         axis=1)
        requests.destination = requests.apply(lambda request: (distances.sample(1, weights='p_destination').index[0]
                                                               if request.dist <= 1000 else
