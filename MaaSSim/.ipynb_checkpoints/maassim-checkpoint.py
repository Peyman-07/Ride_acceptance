...
class Simulator:
    ...
    def myinit(self, **kwargs):
        ...
        if self.params.get('use_fixed_postions',False): #f#
            if len(self.vehicle_fixed_positions)==0:
                self.vehicle_fixed_positions = pd.Series(self.inData.vehicles.pos.values.copy(),
                                                 index=self.inData.vehicles.index.copy())
            else:
                self.vehicles.pos = self.vehicle_fixed_positions
        self.sim_start = None
        #print(f"position of vehicle 1 is {self.vehicles.loc[1].pos}")
